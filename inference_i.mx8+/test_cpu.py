...
if __name__ == '__main__':
    ...
    for path in os.listdir('Validation/images/'):
        ...
        interpreter.set_tensor(input_index, np.reshape(image, (1, 1200,  800,    3)))
        ...
        labels = interpreter.get_tensor(interpreter.get_output_details()[1]["index"])
        scores = interpreter.get_tensor(interpreter.get_output_details()[2]["index"])
        inferences.append(inference_time)
        label = labels[0][np.argmax(scores.flatten())]
        print(f"{path}: {label}")        
    print(psutil.Process(os.getpid()).memory_info().rss / 1024 ** 2)
    print(f"mean inference time {np.mean(inferences)}")
